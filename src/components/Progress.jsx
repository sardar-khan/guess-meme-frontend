    ...
    return (
        <div>
            <div className='flex justify-between items-end w-full mt-[7px] md:mt-[15px]'>
                ...
                <h2 className='mb-[8px] text-[12px] md:!text-[20px]'>{title}: {pusherProgress ? parseFloat(pusherProgress).toFixed(0) : parseFloat(progress).toFixed(0)}%</h2>
            </div>

            <div className='relative overflow-hidden bg-[#dbdae4] h-[20px]'>
                <div
                    className='absolute bg-[#15C570] h-full '
                    style={{ width: `${pusherProgress ? parseFloat(pusherProgress).toFixed(0) : parseFloat(progress).toFixed(0)}%` }}
                ></div>
            </div>
        </div>
    )
